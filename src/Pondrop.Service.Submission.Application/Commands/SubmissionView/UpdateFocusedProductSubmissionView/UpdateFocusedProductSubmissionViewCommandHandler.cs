...
public class UpdateFocusedProductSubmissionViewCommandHandler : IRequestHandler<UpdateFocusedProductSubmissionViewCommand, Result<int>>
{
    ...
    public UpdateFocusedProductSubmissionViewCommandHandler(
    ...
    public async Task<Result<int>> Handle(UpdateFocusedProductSubmissionViewCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var submissionsTask =  _submissionCheckpointRepository.QueryAsync($"SELECT * FROM c WHERE c.id = '{command.SubmissionId}' AND c.submissionTemplateId in ('{_priceTemplateConfig.Id}','{_shelfStockLevelsTemplateConfig.Id}','{_quickPriceAndQuantityTemplateConfig.Id}')");
            ...
            var tasks = submissionsTask.Result.Select(async i =>
            {
                ...
                try
                {
                    ...
                    var storeVisitTask = _storeVisitCheckpointRepository.QueryAsync($"SELECT * FROM c WHERE c.id = '{i.StoreVisitId}'");
                    var submissionTemplateTask = _submissionTemplateCheckpointRepository.QueryAsync($"SELECT * FROM c WHERE c.id = '{i.SubmissionTemplateId}'");

                    await Task.WhenAll(storeVisitTask, submissionTemplateTask);
    ...
    private static string FailedToMessage(UpdateFocusedProductSubmissionViewCommand command) =>
